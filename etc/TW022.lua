...
require"QL_beta"
VERSION='0.2.2'
log=getScriptPath().."\\".."Tradewind.log" --getTradeDate()..
ini_file=getScriptPath().."\\".."tw022_ini.lua"
dofile ( ini_file )
----------------------------------
...
--	Vane_class=getSecurityInfo("",(sec.Vane or Vane)).class_code or ""
	...
	sec.tradingstatus = tonumber(getParamEx (sec.class,sec.name, "TRADINGSTATUS").param_value)
	info=""
...
--or tonumber(sec.tradingstatus) == 1
if isConnected ~= 0 and not sec.stopped  then  -- если бумага не торгуется - графики не считываем, в стакан не заглядываем, позиции не открываем и не закрываем
	...
	--Получаем данные индикатора "Bollinger"
	chart_bb=tostring("bb_"..sec.name)
bb01,bbu1,bbl1=GetGraphValueByCandle(chart_bb, 1, 0),GetGraphValueByCandle(chart_bb, 1, 1),GetGraphValueByCandle(chart_bb, 1, 2)
bb02,bbu2,bbl2=GetGraphValueByCandle(chart_bb, 2, 0),GetGraphValueByCandle(chart_bb, 2, 1),GetGraphValueByCandle(chart_bb, 2, 2)
		--текущий мувинг
bb00,bbu0,bbl0=GetGraphValueByCandle(chart_bb, 0, 0) or bb01,GetGraphValueByCandle(chart_bb, 0, 1) or bbu1,GetGraphValueByCandle(chart_bb, 0, 2) or bbl1
sec.BFactor0 = math_round(bbu0.close/sec.step,2) - math_round(bbl0.close/sec.step,2)
sec.BFactor1 = math_round(bbu1.close/sec.step,2) - math_round(bbl1.close/sec.step,2)
if sec.BFactor0 - sec.BFactor1 > 0 and sec.BFactor0 > sec.channel then --фильтр по Боллинджеру
BBlock = "BB OPEN" 	else BBlock = "BB LOCK" end
...
if sec.enabled then	--в режиме советника  позиции не открываем (но закрыть можем)
	...
	if  (stime>100030 and stime<183000) or (stime>190030 and stime<233000) then
		if  OpenLong and balance<(sec.trade_volume or trade_volume) and not StopSignalLong and BBlock =="BB OPEN" then --*sec.lot
		...
		if  OpenShort and balance>(sec.trade_volume or trade_volume)*-1 and not StopSignalShort and BBlock == "BB OPEN" then --*sec.lot
			if sec.market then
	...
	elseif sec.BFactor0<sec.BFactor1 then SEMAFOR = "WAY BLOCKED"
	end
...
info = info.." "..math_round(sec.BFactor0,2).." | "..math_round(sec.BFactor0-sec.BFactor1,2).." | "..BBlock..
" | "..status.." | "..(sec.Vane or Vane).." | "..sec.class.." | "..tickerstatus
end -- sec.stopped?
if sec.stopped then priceinfo,varmargin,balance = 0,0,0
Storm,Scud,Rudder,Stream,trend_detector,SEMAFOR,info = "NO_DATA","NO_DATA","NO_DATA","NO_DATA","NO_DATA","NO_DATA","STOPPED"
...
if balance>0 then t2t:SetColor(key,"POSITION",LIGHT_GREEN) elseif balance<0 then t2t:SetColor(key,"POSITION",LIGHT_RED) else t2t:SetColor(key,"POSITION",DEFAULT_COLOR) end
if Storm=="UPTREND" then t2t:SetColor(key,"ШТОРМ",LIGHT_GREEN) elseif Storm=="DOWNTREND" then t2t:SetColor(key,"ШТОРМ",LIGHT_RED) else t2t:SetColor(key,"ШТОРМ",DEFAULT_COLOR) end
if Scud=="UPTREND" then t2t:SetColor(key,"ШКВАЛ",LIGHT_GREEN) elseif Scud=="DOWNTREND" then t2t:SetColor(key,"ШКВАЛ",LIGHT_RED) else t2t:SetColor(key,"ШКВАЛ",DEFAULT_COLOR) end
if Rudder=="UPTREND" then t2t:SetColor(key,"ШТУРВАЛ",LIGHT_GREEN) elseif Rudder=="DOWNTREND" then t2t:SetColor(key,"ШТУРВАЛ",LIGHT_RED) else t2t:SetColor(key,"ШТУРВАЛ",DEFAULT_COLOR) end
if Stream=="UPTREND" then t2t:SetColor(key,"ТЕЧЕНИЕ",LIGHT_GREEN) elseif Stream=="DOWNTREND" then t2t:SetColor(key,"ТЕЧЕНИЕ",LIGHT_RED) else t2t:SetColor(key,"ТЕЧЕНИЕ",DEFAULT_COLOR) end
if (last-net_price)*balance>0 then t2t:SetColor(key,"PROFIT",LIGHT_GREEN) elseif (last-net_price)*balance<0 then t2t:SetColor(key,"PROFIT",LIGHT_RED) else t2t:SetColor(key,"PROFIT",DEFAULT_COLOR) end
if varmargin>0 then t2t:SetColor(key,"VARMARGIN",LIGHT_GREEN) elseif varmargin<0 then t2t:SetColor(key,"VARMARGIN",LIGHT_RED) else t2t:SetColor(key,"VARMARGIN",DEFAULT_COLOR) end

end  -- если нет коннекта или бумага не торгуется
...
function math_round( roundIn , roundDig ) -- первый аргумент - число которое надо округлить, второй аргумент - количество символов после запятой.
     ...
     return ( math.floor( ( (roundIn or 0) * mul ) + 0.5 )/mul )
end
----------------------------
-- определение позиции
...
--if magap0>magap1 and magap1>magap2  then
	if tonumber(s0.close) > tonumber(s1.close) and tonumber(s1.close) > tonumber(s2.close) 	and tonumber(s0.close)>tonumber(l0.close) then
	result="UPTREND"
	elseif tonumber(s0.close) < tonumber(s1.close) and tonumber(s1.close) < tonumber(s2.close) and tonumber(s0.close)<tonumber(l0.close) then
	result="DOWNTREND"
	end
--end
return result
